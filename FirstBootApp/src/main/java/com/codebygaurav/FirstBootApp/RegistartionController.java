package com.codebygaurav.FirstBootApp;
...
import com.codebygaurav.FirstBootApp.dao.UserRepo;
...
public class RegistartionController {
	...
	/*
	 * @RequestMapping("/userDetail") public ModelAndView userInfo(UserRegistration
	 * reg) {
	 * 
	 * ModelMap model = new ModelMap(); model.addAttribute("name", reg.getUName());
	 * model.addAttribute("email", reg.getEmailId()); model.addAttribute("gender",
	 * reg.getGender()== Integer.parseInt("1")? "Male":"Female");
	 * model.addAttribute("hobbies", reg.getHobby());
	 * 
	 * 
	 * ModelAndView mv = new ModelAndView("user-details"); mv.addObject("regObj",
	 * model); return mv; }
	 */	
	...
	public ModelAndView selectRecords(UserRegistration reg){
		...
		model.put("hobbies", reg.getHobby());
